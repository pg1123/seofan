...
class DedeTagParse
{
    ...
    function ParseTemplet()
    {
        ...
        //echo '<br/>解析模板中，创建了一个 DedeAttributeParse对象 $cAtt<br />';
        $cAtt = new DedeAttributeParse();
        $cAtt->charToLow = $this->CharToLow;

       // echo '<br/>遍历模板字符串，请取标记及其属性信息 $cAtt<br />';
        //遍历模板字符串，请取标记及其属性信息
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                //echo "<xmp>$attStr</xmp>\r\n";
                //echo '<br/>得到了属性字符串'.$attStr.'<br/>';
                //echo '<br/>调用SetSource($attStr)<br/>';
                $cAtt->SetSource($attStr);
               // var_dump($cAtt->cAttributes->Items);
               // echo '*******************';
                //var_dump($this->CTags);
                //echo '*******************';
                if($cAtt->cAttributes->GetTagName()!='')
                {
                    ...
                    //echo "<font color=blue>$this->Count</font><br/>";
                    $CDTag = new DedeTag();
                    
                   // echo '<br/>'.__LINE__.'new 了一个DedeTag对象<br/>';
                    
                    $CDTag->TagName = $cAtt->cAttributes->GetTagName();
                   // echo '<br/>'.__LINE__.$cAtt->cAttributes->GetTagName().'给了tag的tagname的属性<br/>';
                    
                    $CDTag->StartPos = $sPos;
                    $CDTag->EndPos = $i;
                    $CDTag->CAttribute = $cAtt->cAttributes;
                    //var_dump($cAtt->cAttributes);
                   ...
                   // echo '<hr />';
                   // echo "<br/><font color=blue>ctags dump</font><br/>";
                  //  var_dump($this->CTags);
...
class DedeAttribute
{
    ...
    function GetAtt($str)
    {
        ...
        if($str=="")
        {
            //echo '<font color="red">键值不存在</font>';
            return "";
        }
        if(isset($this->Items[$str]))
        {
            //echo '<font color="red">'. $this->Items[$str].'</font>';
            return $this->Items[$str];
        }
        else
        {
            //echo '<font color="red">该属性不存在</font>';
            return "";
        }
    }
...
class DedeAttributeParse
{
    ...
    {
        //echo '<br/>'.__LINE__.'SetSource($attStr),new了一个DedeAttribute对象DedeAttribute <br/>';
        ...
        */
        ...
        {
            //echo '<br/>'.__LINE__.'SetSource($attStr),调用了 ParseAttribute <br/>';
            $this->ParseAttribute();
        }
    }
    ...
    function ParseAttribute()
    {
        ...
        //echo '<br/>'.__LINE__.'<em>开始解析属性</em>ParseAttribute函数执行 <br/>';
        // 获得Tag的名称，解析到 cAtt->GetAtt('tagname') 中
        for($i=0; $i<$strLen; $i++)
        {            
            if($this->sourceString[$i]==' ')
            {
                //echo '<br/>'.__LINE__.'ParseAttribute for循环中的if分支 <br/>';
        ...
        if(!$hasAttribute)
        {
            //echo '<br/>'.__LINE__.'ParseAttribute 不存在属性列表的情况 <br/>';
            $this->cAttributes->Count++;
            $tmpvalues = explode('.', $tmpvalue);
            $this->cAttributes->Items['tagname'] = ($this->charToLow ? strtolower($tmpvalues[0]) : $tmpvalues[0]);
            if(isset($tmpvalues[1]) && $tmpvalues[1]!='')
        ...
        //echo '<br/>'.__LINE__.'ParseAttribute 如果字符串含有属性值，遍历源字符串,并获得各属性 <br/>';
        ...
        for($i; $i<$strLen; $i++)
        {
            ...
            {
                //echo '<br/>'.__LINE__.'ParseAttribute 查找属性的限定标志 <br/>';
                switch($d)
                {
                    case ' ':
                        break;
            ...
            {
               // echo '<br/>'.__LINE__.'ParseAttribute $startdd==1 <br/>';
